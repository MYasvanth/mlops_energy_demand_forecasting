...
This module implements XGBoost for time series forecasting using the Strategy pattern.
Features:
...
class XGBoostTimeSeriesModel(BaseTimeSeriesModel):
    ...
    def _train_on_features(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        """
        logger.info("Training XGBoost on pre-engineered features...")
        ...
        self.is_fitted = True
        logger.info(f"XGBoost training completed. Train MAE: {self.train_metrics.get('mae', float('inf')):.4f}")

    def fit(self, X_train: pd.DataFrame, y_train: pd.Series,
            X_val: Optional[pd.DataFrame] = None,
            y_val: Optional[pd.Series] = None) -> 'XGBoostTimeSeriesModel':
        """
        ...
        # If X_train has raw data with target column, we need to engineer features
    ...
    def _tune_hyperparameters(self) -> None:
        ...
        """
        logger.info(f"Tuning XGBoost hyperparameters with {self.n_trials} trials...")
    ...
    def _log_artifacts(self) -> None:
        ...
        # Save model
        model_path = f"{self.name}_model.json"
...
# Register the model
ModelRegistry.register('xgboost', XGBoostTimeSeriesModel)

#!/usr/bin/env python3
"""
Deepchecks Monitoring Script for Energy Demand Forecasting

This script runs the Deepchecks monitoring pipeline for data drift detection,
model performance monitoring, and data quality assessment.
"""
...
from src.monitoring.deepchecks_monitoring import MonitoringPipeline
...
def main():
    """
    Main function to run the deepchecks monitoring pipeline.
    """
    parser = argparse.ArgumentParser(description='Run Deepchecks monitoring for energy demand forecasting')
    ...
    parser.add_argument('--output-dir', type=str, default='reports/monitoring_deepchecks',
    ...
    try:
        ...
        logger.info("Initializing deepchecks monitoring pipeline...")
        ...
        logger.info("Running deepchecks monitoring cycle...")
        ...
        # Print results
        print("\n" + "="*60)
        print("DEEPCHECKS MONITORING RESULTS")
        print("="*60)
        ...
        # Print HTML report paths for UI viewing
        print("\nHTML Reports generated (open in browser to view UI):")
        if results['data_quality'].get('report_path'):
            print(f"  - Data Quality: file://{Path(results['data_quality']['report_path']).resolve()}")
        if results['data_drift'].get('report_path'):
            print(f"  - Data Drift: file://{Path(results['data_drift']['report_path']).resolve()}")
        if results['model_performance'] and results['model_performance'].get('report_path'):
            print(f"  - Model Performance: file://{Path(results['model_performance']['report_path']).resolve()}")

        logger.info("Deepchecks monitoring completed successfully")

    except Exception as e:
        logger.error(f"Deepchecks monitoring failed: {str(e)}")
